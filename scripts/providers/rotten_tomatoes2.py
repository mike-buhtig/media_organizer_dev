...
#                      - Overview: Prioritize <rt-text data-qa="synopsis-value">, then JSON-LD description.
#                      - Air Date: Prioritize <rt-text data-qa="item-value"> (Release Date), then mpscall, then JSON-LD.
#                      Ensured all extracted fields handle missing/empty values gracefully.
# [2.2.3] - 2025-05-21: Corrected output filename to 'rotten_tomatoes2.json' to match
#                      Season_Episode_builder.py's expected provider filename.
# [2.2.2] - 2025-05-21: Improved search page parsing. Now directly finds all '/tv/' URLs
#                      and differentiates main series from specials using series_slug,
#                      bypassing reliance on 'rt-search-results-list' container.
# [2.2.1] - 2025-05-21: Added extensive DEBUG logging to search and URL identification.
#                      Temporarily set provider_logger to DEBUG level for troubleshooting.
# [2.2.0] - 2025-05-21: Refactored to prioritize JSON-LD for episode/special data.
#                      Ensured output matches tvmaze.py format (flat episode fields).
#                      Implemented robust error handling for missing data.
#                      Added genre and MPA-rating extraction.
#                      Removed Selenium dependency as it's no longer needed for core data.
#                      Integrated standard Python logging.
# [2.1.7] - 2025-05-10: Used <rt-text slot="content"> for synopsis, <rt-text slot="metadataProp"> for air date, minimized selenium
# [2.1.0] - 2025-05-09: Initial version for v2.x.x, focused on new RT structure.
...
def get_metadata(title: str, config: ConfigParser) -> None:
    ...
    temp_file = os.path.join(temp_folder, "rotten_tomatoes2.json") 
    ...
    # (flat title, overview, id, air_date at episode level)
    output_data = {
        "series_name": title,
        "seasons": [],
        "genres": [],
    ...
    try:
        ...
        # This is more robust than relying on a specific container which might change.
                ...
                if mpscall_script:
                    ...
                    if match:
                        try:
                            ...
                            elif mps_data.get("cag[mpaa_rating]") and not output_data["mpa_rating"]:
